...
...
```
- 分隔头格式固定为 `[文件i/共N] 文件: <名>（页 p/共M）`（N=文件总数，M=该文件页数），文件序号与页码不混淆
...
- 列边界算法：对所有文本框的 x1 做一维聚类（gap=12）→ 列中心；单元格 = 行带 × 列带，文本按框左缘 x1 就近落入列带（距离相等取左侧列）
- 已知局限：右对齐数值列（x1 随位数浮动）可能碎列——输出尾部附提示"列结构可能不精确，如需精确请用 --json 由 LLM 按 box 推理"
